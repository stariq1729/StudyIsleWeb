...
namespace StudyIsleWeb
{
    public partial class SubjectDetails : System.Web.UI.Page
    {
        ...
        {
            string sql = $@"SELECT DISTINCT y.YearId, y.YearName FROM Years y 
            ...
            rptYears.DataSource = GetData(sql);
            rptYears.DataBind();
        }

        private void BindChapters(string sid)
        {
            string sql = $"SELECT ChapterId, ChapterName FROM Chapters WHERE SubjectId = {sid} AND IsActive = 1";
            rptChapters.DataSource = GetData(sql);
